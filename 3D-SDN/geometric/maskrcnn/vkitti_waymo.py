...
from datasets.vkitti_utils_waymo import get_tables, get_lists
...
class VKittiConfig(Config):
    ...
    GPU_COUNT = 1
    if GPU_COUNT>0:
        device_ids = [_ for _ in range(GPU_COUNT)]
...
class VKittiDataset(utils.Dataset):
    def load_vkitti(self, dataset_dir, subset):
        ...
        self.list_images = json.load(open(os.path.join(
            os.path.dirname(__file__), "assets/vkitti_maskrcnn_waymo_{}.json".format(subset))))
    ...
    def load_mask(self, image_id):
        ...
        inst_map = np.apply_along_axis(
            lambda a: self.table_inst[(worldId, sceneId, a[0], a[1], a[2])] if (worldId, sceneId, a[0], a[1], a[2]) in self.table_inst else 0, 2, inst_map) # if class RGB color is find in the dictionary return class id, else return undefined 
        ...
        # ids = ids[ids // 5000 != 31]      # filter out trucks
        # ids = ids[ids // 5000 != 18]      # filter out ped
        # ids = ids[ids // 5000 != 30]      # filter out trailer
        ids = ids[(ids // 5000 == 11) | (ids // 5000 == 32)]      # pick car or van
        ...
        class_ids = np.fromiter(map(lambda x: {11: 1, 32: 2}[x // 5000], ids), dtype=np.int)  # dont care about trucks no more
