...
class TrainDataset(torchdata.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_per_gpu):
            ...
            segm = np.apply_along_axis(lambda a: self.table_segm[(worldId, sceneId, a[0], a[1], a[2])] if (worldId, sceneId, a[0], a[1], a[2]) in self.table_segm else 0, 2, segm) # if class RGB color is find in the dictionary return class id, else return undefined 
            segm = segm.astype(np.uint8)

            # add data augmentation: color jittering
            img = self.img_jitter(img)
            img = np.array(img)
            img = img[:,:,:3] # for temp only, remove alpha channel
            ...
            # print(batch_images[i][:, :img.shape[1], :img.shape[2]].shape)
            # print(img.shape)
